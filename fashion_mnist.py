...
"""In this python script, CNN, FC and LSTM models are utilized to classify the fashion_mnist
   dataset."""
...
class FashionMnist(Mnist):
    ...
    """This class is the child of Mnist class and all of its mathods and attributes are 
       feasible here. The only difference is the label_map which defines some clothes
       rather than numbers between 0 and 9."""
       
    def __init__(self, kind, X_train, y_train, X_test, y_test):
        
        super().__init__(kind, X_train, y_train, X_test, y_test)
...
fashion_mnist = FashionMnist("CNN", X_train, y_train, X_test, y_test)

# Type of Classification model and shape of input data
kind, label_map, num_classes = fashion_mnist.data_desciption()
N,T,D = X_train.shape
...
fashion_mnist.loss_accuracy(model_dict, 5)

# Predicting the test data
y_pred = np.argmax(model.predict(X_test),axis = 1)

# Plotting the Confusion matrix using the confusion_matrixx function in the Mnist class
fashion_mnist.confusion_matrixx(y_pred,num_classes = num_classes, fontsize = 14,thresh = 50,cmap = plt.cm.Blues)

# Plotting Wrong predictions using wrong_pred class in the Mnist class
fashion_mnist.wrong_pred(y_pred)
